    ...
    def accuracy_checker(self, loader, model):
        ...
        with torch.no_grad():
            for data in tqdm(loader):
                ...
                pixels += torch.numel(predicts)
                dice += self.dice_coeff(predicts, labels)
        model.train()
        print("Accuracy of the network on the test images: %d %% " % (100 * correct / total))
        print("Dice score of the network on the test images: %d %% " % (100 * dice / total))
...
class Mydata(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask[mask == 255.0] = 1
        if self.transform:
            augmentations = self.transform(img, mask)
...
def main():
    ...
    util.check_accuracy(validations_dataloader, model, DEVICE)
